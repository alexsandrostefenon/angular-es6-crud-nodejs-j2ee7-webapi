...
/**
	...
	public static String compareIgnore(Message messageRef, Message message, String[] fieldsIgnore) throws Exception {
		String ret = null;

		for (String name : Message.fixedFieldNames) {
			if (Utils.findInList(fieldsIgnore, name) < 0) {
				String valRef = messageRef.getFieldData(name);
				String val = message.getFieldData(name);
