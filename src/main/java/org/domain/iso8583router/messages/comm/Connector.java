...
public class Connector implements Logger, ModuleManager {
	...
	// plugin manager
	private final Map<String, Module> listModules = new HashMap<String, Module>();
	private final ArrayList<Semaphore> listSemaphores = new ArrayList<Semaphore>();
	// logger
	SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd-HHmmss.SSS");
	private int logLevel = Logger.LOG_LEVEL_WARNING;
	...
	public List<Message> getMessagesRouteRef() {
	...
	public synchronized void log(int logLevel, String header, String text, Object data) {
...
//				entityManager.clear();
	...
	private Module loadModule(String className) throws Exception {
		...
		this.listModules.put(className, instance);
		this.listSemaphores.add(new Semaphore(1));
		return instance;
	}
	
	private Module getModule(String name) throws Exception {
		Module plugin = listModules.get(name);
		
		if (plugin == null) {
			try {
				plugin = loadModule(name);
			} catch (Exception e) {
				throw new Exception(String.format("Connector.startModule(%s) : %s", name, e.getMessage()));
			}
		}
	...
	public ModuleInfo getInfoModule(String name) throws Exception {
		Module plugin = this.getModule(name);
		return new ModuleInfo(name, plugin);
	}
	...
	private long commIn(Message messageIn, Message messageOut) throws Exception {
		long rc = -1;
		String className = messageIn.getModule();
		
		if (className != null) {
			String msgType = messageIn.getMsgType();
			Module plugin = getModule(className);
			// TODO : storage before execute
			rc = plugin.execute(messageIn);
			messageIn.setMsgTypeResponse(msgType);
			// TODO : storage after execute
		} else {
	...
	public long route(Message messageIn, ISO8583RouterCommConf conf) throws Exception {
		...
		String module = null;
		Message ref = Message.getFirstCompatible(this.messagesRouteRef, messageIn, Connector.fieldsRouteMask, false, this, "Connector.route");
	...
	public void start() {
		...
		for (ISO8583RouterCommConf conf : this.confs) {
			...
			try {
				...
				if (conf.getListen() == false && conf.getPermanent() == false && conf.getDirection() == Utils.CommRequestDirection.CLIENT_TO_SERVER) {
				...
				for (int j = 0; j < conf.getMaxOpenedConnections(); j++) {
					if ((conf.getListen() == true && conf.getDirection() == Utils.CommRequestDirection.CLIENT_TO_SERVER)) {
						try {
					...
					} else if (conf.getDirection() == Utils.CommRequestDirection.BIDIRECIONAL) {
	...
	public long commOut(Message messageOut, Message messageIn) {
		long rc = -1;
	...
	public static Connector daemon(String[] args) {
		...
		Connector manager = Connector.getInstance();
		manager.setEntityManager(entityManagerFactory.createEntityManager());
		manager.start();
		return manager;
	}
	...
	@Override
	public long execute(String pluginName, Object data) {
		// TODO Auto-generated method stub
		return 0;
	}

	@Override
	public ModuleInfo[] getPluginsInfo() {
		// TODO Auto-generated method stub
		return null;
	}
