CREATE TABLE account (
	...
	PRIMARY KEY(crud_group_owner,id),
	unique(bank,agency,number)
);

CREATE TABLE person (
	crud_group_owner integer,
...
CREATE TABLE request_nfe ( -- nota fiscal
	...
	person integer NOT NULL references person,-- emit/dest primeiro cadastro de person para esta crud_group_owner
	...
	PRIMARY KEY(crud_group_owner,request),
	FOREIGN KEY(crud_group_owner,request) REFERENCES request(crud_group_owner, id)
);
...
CREATE TABLE request_freight ( -- frete
	...
	PRIMARY KEY(crud_group_owner,request),
	FOREIGN KEY(crud_group_owner,request) REFERENCES request(crud_group_owner, id),
	UNIQUE(crud_group_owner,request)
);
...
CREATE TABLE request_payment (
	...
	PRIMARY KEY(crud_group_owner,id),
	FOREIGN KEY(crud_group_owner,request) REFERENCES request(crud_group_owner, id),
	FOREIGN KEY(crud_group_owner,account) REFERENCES account(crud_group_owner, id)
);
...
CREATE OR REPLACE FUNCTION account_balance_after_date(_crud_group_owner integer, _account integer, date_ref timestamp, ref_id integer, diff numeric(9,2), request_type integer) RETURNS integer AS $account_balance_after_date$
DECLARE
BEGIN
	  ...
	  UPDATE request_payment SET balance = balance + diff
	  	WHERE
	  		crud_group_owner = _crud_group_owner AND
...
BEGIN
		IF (TG_OP = 'INSERT') THEN
			  SELECT INTO request_rec * FROM request WHERE request.id = NEW.request;
				PERFORM account_balance_after_date(NEW.crud_group_owner, NEW.account, GREATEST(NEW.due_date, NEW.payday), NEW.id, NEW.value, request_rec.type);
				RETURN NEW;
		ELSEIF (TG_OP = 'UPDATE') THEN
			  SELECT INTO request_rec * FROM request WHERE request.id = OLD.request;
				PERFORM account_balance_after_date(OLD.crud_group_owner, OLD.account, GREATEST(OLD.due_date, OLD.payday), OLD.id, OLD.value * (-1.0), request_rec.type);
				PERFORM account_balance_after_date(NEW.crud_group_owner, NEW.account, GREATEST(NEW.due_date, NEW.payday), NEW.id, NEW.value, request_rec.type);
				RETURN NEW;
...
BEGIN
		...
		FOR rec_payment IN SELECT id,GREATEST(payday,due_date) as date_ref,balance FROM request_payment WHERE crud_group_owner = NEW.crud_group_owner AND account = NEW.account AND GREATEST(payday,due_date) <= date_new ORDER BY date_ref desc,id desc LIMIT 1 LOOP
